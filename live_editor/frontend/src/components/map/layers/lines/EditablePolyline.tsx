...
import { useLeaflet, PolylineProps } from 'react-leaflet';

export type EditablePolylineProps = 
  PolylineProps & {
...
function editablePolylineCtor(map: any, props: EditablePolylineProps ) {   
    ...
    const createMarkers = () => {
        ...
        for (const index in positions) {       
            markers[index] =
                 new Marker(positions[index], {'draggable': true, 'icon': markerIcon}).addTo(map);
